...
async function verifyRecipient(req, res, next) {
  try {
    const recipientPhone = String(req.body.phone || '').trim();
    ...
    if (!recipient) {
      return res.status(404).json({
        success: false,
        message: 'Kaduna Only wallet not found'
      });
    }
    ...
    const maskedPhone =
      phone.length >= 7
        ? `${phone.slice(0, 4)}${'*'.repeat(Math.max(phone.length - 7, 3))}${phone.slice(-3)}`
        : phone;
...
module.exports={me,transactions,initializePaystack,verifyPaystack,paymentHistory,setPin,verifyRecipient,transfer};
